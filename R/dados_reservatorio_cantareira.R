...
#'
#' @export
#' @param codigo_reservatorio_cantareira Codigo do reservatorio a ser buscado.
...
#'
#' @examples
#' dados_reservatorio_cartareira(29001, "2000-01-01", Sys.Date())

dados_reservatorio_cantareira <- function(codigo_reservatorio_cantareira, data_inicial = "1980-01-01", data_final = Sys.Date()){
  url <- glue::glue("https://www.ana.gov.br/sar0/MedicaoCantareira?dropDownListReservatorios={codigo_reservatorio_cantareira}&dataInicial={format.Date(data_inicial, \'%d\')}%2F{format.Date(data_inicial, \'%m\')}%2F{format.Date(data_inicial, \'%Y\')}&dataFinal={format.Date(data_final, \'%d\')}%2F{format.Date(data_final, \'%m\')}%2F{format.Date(data_final, \'%Y\')}")

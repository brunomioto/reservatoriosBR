...
ONS_EAR_subsistemas <- function(ano_inicial=2000, ano_final=format(Sys.Date(), "%Y")){
  ...
  historico_ear_clean <- historico_ear %>% 
    dplyr::mutate(subsistema = recode(id_subsistema,
                                      N = "Norte",
                                      NE = "Nordeste",
    ...
    dplyr::select(data, subsistema, ear_max_subsistema, ear_verif_subsistema_mwmes, ear_verif_subsistema_percentual)
